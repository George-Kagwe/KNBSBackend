...
from education.models import Approved_Degree_Diploma_Programs, \
    ...
    Population_Distribution_Above_Three_School_Attendance, Distribution_AboveThreeYears_Training, \
    Distribution_AboveThreeYears_Highestlevel_Reached, Distribution_SixThirteen_By_SchoolType, \
    Distribution_AboveFifteen_Ability_ReadWrite, Distribution_Three_Twentyfour_SchoolAttendance, \
    Gross_Attendance_Ratio_By_Level, Net_Attendance_Ratio_By_Level, Distribution_Highest_Education_Qualification
from health.models import SubCounty, Sectors, Counties
...
def polytechnicCategorySubcounty(request):
    ...
    if poly_cat:
        for polytechnic in poly_cat:
            ...
            if subcounty:
                for sc in subcounty:
                    c = {'county': county.county_name, 'sub_county': sc.subcounty_name, 'pub_lic': polytechnic.public,
                         'pri_vate': polytechnic.private, 'year': polytechnic.year}
                    polytechnics.append(c)
    else:
        pass
...
############################################KIHBIS II############################################
####################################Population_Distribution_Above_Three_School_Attendance####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def populationDistribution(request):
    pop_dist = Population_Distribution_Above_Three_School_Attendance.objects.all()

    distribution = []

    if pop_dist:
        for dist in pop_dist:
            county = Counties.objects.get(county_id=dist.county_id)

            c = {'county': county.county_name, 'currently_attending': dist.currently_attending,
                 'not_attending': dist.not_attending, 'no_of_individuals': dist.no_of_individuals}
...
####################################Distribution_AboveThreeYears_Training####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def distributionAboveThree(request):
    dist_above = Distribution_AboveThreeYears_Training.objects.all()

    distribution = []

    if dist_above:
        for dist in dist_above:
            county = Counties.objects.get(county_id=dist.county_id)

            c = {'county': county.county_name, 'ever_attended': dist.ever_attended,
                 'never_attended': dist.never_attended, 'not_stated': dist.not_stated, 'no_of_individuals': dist.no_of_individuals}
...
####################################Distribution_AboveThreeYears_Highestlevel_Reached####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def distributionAboveThreeReached(request):
    dist_above = Distribution_AboveThreeYears_Highestlevel_Reached.objects.all()
    ...
    if dist_above:
        for dist in dist_above:
            ...
            c = {'county': county.county_name, 'pre_primary': dist.pre_primary,
                 'primary': dist.primary, 'post_primary': dist.post_primary, 'secondary': dist.secondary,
                 'college': dist.college, 'university': dist.university,
                 'madrassa_duksi': dist.madrassa_duksi, 'other': dist.other,
                 'not_stated': dist.not_stated, 'no_of_individuals': dist.no_of_individuals}
...
####################################Distribution_SixThirteen_By_SchoolType####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def distributionSchoolType(request):
    dist_school = Distribution_SixThirteen_By_SchoolType.objects.all()

    distribution = []

    if dist_school:
        for dist in dist_school:
            county = Counties.objects.get(county_id=dist.county_id)

            c = {'county': county.county_name, 'day': dist.day,
                 'boarding': dist.boarding, 'not_stated': dist.not_stated, 'type': dist.type}
...
####################################Distribution_AboveFifteen_Ability_ReadWrite####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def distributionAboveFifteenAbility(request):
    dist_above = Distribution_AboveFifteen_Ability_ReadWrite.objects.all()
    ...
    if dist_above:
        for dist in dist_above:
            ...
            c = {'county': county.county_name, 'literate': dist.literate,
                 'illiterate': dist.illiterate, 'not_stated': dist.not_stated, 'no_of_individuals': dist.no_of_individuals,
                 'gender': dist.gender}
...
####################################Distribution_Three_Twentyfour_SchoolAttendance####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def distributionSchoolAttendance(request):
    dist_school = Distribution_Three_Twentyfour_SchoolAttendance.objects.all()
    ...
    if dist_school:
        for dist in dist_school:
            ...
            c = {'county': county.county_name, 'currently_attending': dist.currently_attending,
                 'not_attending': dist.not_attending, 'no_of_individuals': dist.no_of_individuals, 'age_group': dist.age_group}
...
####################################Gross_Attendance_Ratio_By_Level####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def grossAttendance(request):
    gross_attendance = Gross_Attendance_Ratio_By_Level.objects.all()

    attendance = []

    if gross_attendance:
        for gross in gross_attendance:
            county = Counties.objects.get(county_id=gross.county_id)

            c = {'county': county.county_name, 'pre_primary': gross.pre_primary,
                 'primary': gross.primary, 'secondary': gross.secondary, 'gender': gross.gender}
...
####################################Net_Attendance_Ratio_By_Level####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def netAttendance(request):
    net_attendance = Net_Attendance_Ratio_By_Level.objects.all()

    attendance = []

    if net_attendance:
        for net in net_attendance:
            county = Counties.objects.get(county_id=net.county_id)

            c = {'county': county.county_name, 'pre_primary': net.pre_primary,
                 'primary': net.primary, 'secondary': net.secondary, 'gender': net.gender}
...
####################################Distribution_Highest_Education_Qualification####################################
# All Records
@api_view(http_method_names=['GET'])
@renderer_classes((JSONRenderer,))
def distributionHighest(request):
    dist_highest = Distribution_Highest_Education_Qualification.objects.all()

    attendance = []

    if dist_highest:
        for net in dist_highest:
            county = Counties.objects.get(county_id=net.county_id)

            c = {'county': county.county_name, 'none': net.none,
                 'cpe_kcpe': net.cpe_kcpe, 'kape': net.kape, 'kjse': net.kjse,
                 'kce_kcse': net.kce_kcse, 'kace_eaace': net.kace_eaace, 'certificate': net.certificate,
                 'diploma': net.diploma, 'degree': net.degree, 'post_literacy_cert': net.post_literacy_cert,
                 'other': net.other, 'not_stated': net.not_stated, 'no_of_individuals': net.no_of_individuals}
